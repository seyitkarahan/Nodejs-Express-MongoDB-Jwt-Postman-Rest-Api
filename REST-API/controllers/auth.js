...
const register = async(req, res) => {
    try {
        ...
        if (user) {
            return res.status(500).json({message: "Bu email hesabi zateen bulunmakta!"});
        }

        if (password.length < 6) {
            return res.status(500).json({message: "Parolaniz 6 karakterden kucuk olmali!"});
        }
...
const login = async(req, res) => {
    try {
        ...
        if (!user) {
            return res.status(500).json({message: "Boyle bir kullanici bulunmadi!"})
        }
